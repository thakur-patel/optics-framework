...
class TestPressElementWithIndex:
    ...
    @patch('optics_framework.common.utils.save_screenshot')
    @patch('optics_framework.common.utils.determine_element_type')
    def test_press_element_with_index_int(self, mock_determine_type, mock_save_screenshot, action_keyword, mock_dependencies):
        with patch.object(action_keyword.strategy_manager, 'capture_screenshot', return_value=np.zeros((10,10,3), dtype=np.uint8)):
    ...
    def test_press_element_with_invalid_aoi_parameters(self, action_keyword):
        """Test that press_element raises OpticsError for incomplete AOI parameters (as per StrategyManager.locate)."""
        with patch.object(action_keyword.strategy_manager, 'capture_screenshot', return_value=np.zeros((10,10,3), dtype=np.uint8)):
            # Test with only some AOI parameters provided
            with pytest.raises(OpticsError, match="All AOI parameters.*must be provided together"):
